...
const WORKER_PARENT_PIPE_END_NAME = 'pear-doctor-worker-parent-pipe-end'
const WORKER_PARENT_PIPE_END_DIR = path.join(tmp, WORKER_PARENT_PIPE_END_NAME)
...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    #attachEventListeners () {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach(button => {
        ...
        const listener = (event) => {
          ...
          if (type === 'Worker-Parent-Pipe-End' && action === 'End') this.#workerParentPipeEnd()
          if (type === 'Worker-Parent-Pipe-End' && action === 'Check') this.#workerParentPipeEndCheck()
        }
    ...
    async #resetChecks () {
      ...
      await fs.promises.rm(WORKER_PARENT_PIPE_END_DIR, { recursive: true, force: true })
    }
    ...
    async #workerParentPipeEnd () {
      const dir = WORKER_PARENT_PIPE_END_DIR
      await fs.promises.rm(dir, { recursive: true, force: true })
      await fs.promises.mkdir(dir, { recursive: true })
      await fs.promises.writeFile(path.join(dir, 'index.js'), `
        const fs = require('bare-fs')
      ...
      `)
      await fs.promises.writeFile(path.join(dir, 'package.json'), JSON.stringify({
        name: WORKER_PARENT_PIPE_END_NAME,
        main: 'index.js',
        pear: { type: 'terminal' },
        dependencies: { 'bare-fs': 'latest', 'bare-path': 'latest' }
      }))
      ...
      pipe.end()
      setTimeout(() => this.#workerParentPipeEndCheck(), 1000)
    }

    async #workerParentPipeEndCheck () {
      const outputPath = path.join(WORKER_PARENT_PIPE_END_DIR, 'output.tmp')
      const contents = await fs.promises.readFile(outputPath, 'utf8')
      const lines = contents.split('\n')
      const lastLine = lines[lines.length - 2]

      if (lastLine === 'exited') this.#updateState({ WORKER_PARENT_PIPE_END: true })
    }
