...
console.log('🚀 ~ versions:', await versions())
...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    async #restartClient () {
      ...
      if (error) {
        console.error(error)
        // fail the test if failed to get sidecar pid
        this.#updateState({ LIFECYCLE_RESTART_CLIENT: false })
        return
      }
      if (result.sidecars.length !== 1) { 
        // fail the test if there are more than one sidecar
        this.#updateState({ LIFECYCLE_RESTART_CLIENT: false })
        return
      }
      const sidecarPid = result.sidecars[0]
      if (!sidecarPid) {
        // fail the test if there is no sidecar pid
        this.#updateState({ LIFECYCLE_RESTART_CLIENT: false })
        return
      }
      
      console.log('🚀 ~ extends ~ #restartClient ~ sidecarPid:', sidecarPid, this.state)
      // save sidecar pid, then restart
      if (!this.state.sidecarPid) {
        this.#updateState({ sidecarPid })
        await Pear.restart({ platform: false })
        return
      }
      
      // fail the test if the sidecar pid is the same
      if (this.state.sidecarPid === sidecarPid) {
        this.#updateState({ LIFECYCLE_RESTART_CLIENT: false })
        return
      }
