  ...
  class extends HTMLElement {
        ...
        ${section.items
          .map(
            (item, index) => {
              ...
              const isInactive = !this.state.started || !isActive
              return `
    ...
    async #updatePlatform () {
      ...
      const bootstrapPlatforms = async () => {
        ...
        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'dumping...' })
        await fs.promises.rm(dumpDir, { recursive: true, force: true })
        await fs.promises.mkdir(dumpDir, { recursive: true })
        const dumpChild = spawn(bin, ['dump', '--json', `pear://${version.platform.key}`, dumpDir])
        ...
        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'staging...' })
        const stageChild = spawn(bin, ['stage', '--json', 'dev', dumpDir])
        teardown(async () => stageChild.kill())
        const stageOut = await spawnUntil({ sc: stageChild, tag: 'addendum' })
        const dumpKey = stageOut.key

        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'seeding...' })
        const seedChild = spawn(bin, ['seed', '--json', 'dev', dumpDir])
        teardown(async () => seedChild.kill())
        await spawnUntil({ sc: seedChild, tag: 'announced' })

        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'bootstrapping-1...' })
        ...
        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'bootstrapping-2...' })
      ...
      const platformDir1SeedApp = async () => {
        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'creating-app...' })
        ...
        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'staging-app...' })
        const stageChild = spawn(platformRuntime, ['stage', '--json', 'dev', appDir])
        teardown(async () => stageChild.kill())
        const stageOut = await spawnUntil({ sc: stageChild, tag: 'addendum' })

        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'seeding-app...' })
        const seedChild = spawn(platformRuntime, ['seed', '--json', 'dev', appDir])
        teardown(async () => seedChild.kill())
        await spawnUntil({ sc: seedChild, tag: 'announced' })
      ...
      const platformDir2RunApp = async () => {
        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'creating-run-app...' })
        ...
        await fs.promises.writeFile(path.join(appDir, 'index.js'), `
          ...
          Pear.worker.constructor.RUNTIME = '${platformRuntime}'
        ...
        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'running-app...' })
      ...
      const platformDir1StageNewFile = async () => {
        await fs.promises.writeFile(path.join(dumpDir, `${Date.now()}.tmp`), `${Date.now()}`)
        const platformRuntime = path.join(platformDir1, 'current', BY_ARCH)
        this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'staging-app-again...' })
        const stageChild = spawn(platformRuntime, ['stage', '--json', 'dev', dumpDir])
        teardown(async () => stageChild.kill())
      ...
      // main
      //
      this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'starting...' })
      // await bootstrapPlatforms() // comment out if you want to skip the bootstrap
      const { appLink } = await platformDir1SeedApp(platformDir1)
      const { pipe, updatePromise } = await platformDir2RunApp(platformDir2)
      await platformDir1StageNewFile(dumpDir, platformDir1)

      this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'waiting for update...' })
      const updateRes = await updatePromise
      const isUpdated = updateRes.type === 'pear/updates' && updateRes.app === false
      this.#updateState({ LIFECYCLE_UPDATE_PLATFORM: isUpdated })

      pipe.end()
      this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': 'waiting for close...' })
      await pipeUntilClose({ pipe })
      this.#updateState({ 'status-LIFECYCLE_UPDATE_PLATFORM': undefined })
    }
