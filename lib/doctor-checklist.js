...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    #attachEventListeners() {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach((button) => {
        ...
        const listener = (event) => {
          ...
          if (type === 'Update-App-Desktop' && action === 'Test')
            this.#updateAppDesktopTest()
          if (type === 'Update-App-Desktop' && action === 'Check')
            this.#updateAppDesktopCheck()
          if (type === 'Update-Platform' && action === 'Check')
            this.#updatePlatform()
          if (type === 'Update-Platform-Desktop' && action === 'Test')
            this.#updatePlatformDesktopTest()
          if (type === 'Update-Platform-Desktop' && action === 'Check')
            this.#updatePlatformDesktopCheck()
    ...
    async #sidecars() {
      ...
      const { result, error } = await this.sidecars({
        name: 'pear-runtime',
        flag: '--sidecar'
      })
    ...
    async #updatePlatform() {
      ...
      const platformDir2RunApp = async (appLink) => {
        ...
        await fs.promises.writeFile(
          ...
          pipeIn.resume()
          Pear.worker.constructor.RUNTIME = '${sanitizePath(platformRuntime)}'
    ...
    async #workerParentClose() {
      ...
      setTimeout(() => Pear.Window.self.quit(), 2000)
    }
    ...
    async #userAgentCheck() {
      ...
      this.#updateState({
        MISC_USER_AGENT: text === Pear.config.options.userAgent
      })
    }
    ...
    async #trayCustomIconRun() {
      ...
      await Pear.tray({ icon: 'assets/Pear-white-16px.png' })
    }
    ...
    async #trayCustomMenuRun() {
      ...
      await Pear.tray({ menu: { foo: 'Foo', bar: 'Bar' } }, (key) => {
    ...
    async #trayMultipleRunsRun() {
      ...
      await Pear.tray()
      await Pear.tray({ icon: 'assets/Pear-white-16px.png' })
      await Pear.tray({ menu: { foo: 'Foo', bar: 'Bar' } })
      await Pear.tray(
        {
...
async function platformDir1SeedApp({ onProgress, platformDir1, appDir }) {
  ...
  const stageChild = spawn(
    platformRuntime,
    ['stage', '--json', 'dev', appDir],
    { shell: !!isWindows }
  )
  teardown(async () => stageChild.kill())
  const stageOut = await spawnUntil({ sc: stageChild, tag: 'addendum' })
