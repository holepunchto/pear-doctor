...
const isLocal = config.pearDir.endsWith('pear/pear')
const bin = isLocal
  ? path.join(config.pearDir, '..', 'pear.dev')
  : path.join(config.pearDir, 'bin', isWindows ? 'pear.cmd' : 'pear')
  ...
  class extends HTMLElement {
        ...
        ${section.items
          .map(
            (item, index) => {
              ...
              const isInactive = !this.state.started || !isActive || this.state[`loading-${item.id}`]
              return `
            <div class="list-item " style="margin-left: 1.25rem;">
              ...
              ${item.button ? `<${item.button} id="${item.id}" action="${item.action}" type="${item.type}" class="${isInactive ? 'inactive' : ''}"></${item.button}>` : ''}
              ${item.buttonSecond ? `<${item.buttonSecond} id="${item.id}" action="${item.actionSecond}" type="${item.typeSecond}" class="secondary-button${isInactive ? ' inactive' : ''}"></${item.buttonSecond}>` : ''}
            </div>
    ...
    #attachEventListeners () {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach(button => {
        ...
        const listener = async (event) => {
          const { id, type, action } = event.detail
          this.#updateState({ [`loading-${id}`]: true, [id]: null })
          if (type === 'Nurse') await this.#openNurse()
          if (type === 'Microphone' && action === 'Status') await this.#getMicrophoneStatus()
          if (type === 'Camera' && action === 'Status') await this.#getCameraStatus()
          if (type === 'Screen' && action === 'Status') await this.#getScreenStatus()
          if (type === 'Microphone' && action === 'Access') await this.#getMicrophoneAccess()
          if (type === 'Camera' && action === 'Access') await this.#getCameraAccess()
          if (type === 'Screen' && action === 'Access') await this.#getScreenAccess()
          if (type === 'Zombie-Sidecar' && action === 'Check') await this.#sidecars()
          if (type === 'Trust-Dialog' && action === 'Check') await this.#checkTrustDialog()
          if (type === 'Password-Dialog' && action === 'Check') await this.#checkPasswordDialog()
          if (type === 'Teardown-Normal' && action === 'Check') await this.#teardownNormal()
          if (type === 'Teardown-Force' && action === 'Check') await this.#teardownForce()
          if (type === 'Restart-Platform' && action === 'Restart') await this.#restartPlatform()
          if (type === 'Restart-Platform' && action === 'Check') await this.#restartPlatformCheck()
          if (type === 'Restart-Client' && action === 'Restart') await this.#restartClient()
          if (type === 'Restart-Client' && action === 'Check') await this.#restartClientCheck()
          if (type === 'Update-App' && action === 'Check') await this.#updateApp()
          if (type === 'Update-Worker' && action === 'Check') await this.#updateWorker()
          this.#updateState({ [`loading-${id}`]: false })
        }
    ...
    async #updateApp () {
      ...
      await fs.promises.writeFile(path.join(tmpDir, 'index.js'), `
        Pear.updates((data) => {
          console.log(JSON.stringify({ tag: 'update', data }))
          Pear.exit()
        })
        Pear.versions().then((data) => console.log(JSON.stringify({ tag: 'versions', data })))
      `)
      ...
      const runChildClosePromise = spawnUntilClose({ sc: runChild })
      const versions = await spawnUntil({ sc: runChild, tag: 'versions' })
      ...
      console.log('staging2', bin, tmpDir)
      ...
      runChild.kill()
      await runChildClosePromise

      this.#updateState({ LIFECYCLE_UPDATE_APP: updated.version.length > versions.app.length })
    }
    ...
    async #updateWorker () {
      ...
      await fs.promises.writeFile(path.join(tmpDir, 'index.js'), `
        ...
        const updates = Pear.updates((data) => updatesHandler(data))
        function updatesHandler (data) {
        ...
        pipe.resume()
        Pear.versions().then((data) => pipe.write(JSON.stringify({ tag: 'versions', data })))
      `)
      await fs.promises.writeFile(path.join(tmpDir, 'package.json'), JSON.stringify({
        name,
        main: 'index.js',
      ...
      const versions = await pipeUntil({ pipe, tag: 'versions' })
      console.log('versions', versions)
      ...
      pipe.end()
      await pipeUntilClose({ pipe }) // TODO: the worker process does not kill itself here, need to figure out why
      this.#updateState({ LIFECYCLE_UPDATE_WORKER: updated.version.length > versions.app.length })
    }
...
async function spawnUntilClose ({
  sc
}) {
  return new Promise((resolve) => {
    sc.on('close', () => resolve('closed'))
    sc.on('end', () => resolve('ended'))
  })
}
