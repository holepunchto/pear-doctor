...
customElements.define(
  ...
  class extends HTMLElement {
    constructor () {
      ...
      this.inviteCode = config.fragment && this.#isKeetInvite(config.fragment)
        ...
        ${section.items
          .map(
            ...
            <div class="list-item " style="margin-left: 20px;">
              <input type="checkbox" id="item-${index}" ${this.state[item.id] === true && item.active !== false ? 'checked' : ''} class="check-item ${!this.state.started || item.active === false ? 'inactive' : ''} ${this.state[item.id] === false && item.active !== false ? 'red-cross' : ''}" ${!this.state.started ? 'disabled' : ''}>
              <label for="item-${index}" class="${this.state.started ? '' : 'disabled-label'} label ${!this.state.started || item.active === false ? 'inactive' : ''}">${item.text}</label>
              <info-tooltip info-text="${item.tooltip}"></info-tooltip>
              ${item.link ? `<a href="${this.#parseLink(item.link)}" target="_blank" class="link ${!this.state.started || item.active === false ? 'inactive' : ''}">Link</a>` : ''}
              ${item.button ? `<${item.button} action="${item.action}" type="${item.type}" class="${!this.state.started || item.active === false ? 'inactive' : ''}"></${item.button}>` : ''}
            </div>
          `
          )
          .join('')}
    ...
    #parseLink (link) {
      ...
      if (link.includes(pattern)) return link.replace(pattern, config.applink)
    }
