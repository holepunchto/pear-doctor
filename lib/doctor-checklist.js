...
customElements.define(
  ...
  class extends HTMLElement {
        ...
        ${section.items
          .map(
            ...
            <div class="list-item " style="margin-left: 1.25rem;">
              ...
              ${item.link ? `<action-button id="${item.id}" action="Link" type="Click" href="${item.link}" class="${isInactive ? 'inactive' : ''}" applink="${applink}"></action-button>` : ''}
    ...
    async #updateApp () {
      ...
      this.#updateState({ LIFECYCLE_UPDATE_APP: updated.diff.length > 0 })
    }
