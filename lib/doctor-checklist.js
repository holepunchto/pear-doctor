...
async function bootstrapPlatforms({
  ...
  const stageChild = spawn(bin, ['stage', '--json', 'dev'], {
    cwd: dumpDir,
    shell: !!isWindows
  })
  teardown(async () => stageChild.kill())
  ...
  const seedChild = spawn(bin, ['seed', '--json', 'dev'], {
    cwd: dumpDir,
    shell: !!isWindows
  })
...
async function platformDir1SeedApp({ onProgress, platformDir1, tmpDir }) {
  ...
  const stageChild = spawn(platformRuntime, ['stage', '--json', 'dev'], {
    cwd: tmpDir,
    shell: !!isWindows
  })
  teardown(async () => stageChild.kill())
  ...
  const seedChild = spawn(platformRuntime, ['seed', '--json', 'dev'], {
    cwd: tmpDir,
    shell: !!isWindows
  })
  teardown(async () => seedChild.kill())
