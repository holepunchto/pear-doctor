  ...
  class extends HTMLElement {
    ...
    #attachEventListeners () {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach(button => {
        ...
        const listener = (event) => {
          ...
          if (type === 'Restart-Platform-Timeout' && action === 'Restart') this.#restartPlatformTimeout()
          if (type === 'Restart-Platform-Timeout' && action === 'Check') this.#restartPlatformTimeoutCheck()
    ...
    #getMicrophoneAccess () {
      Pear.media.access.microphone().then((res) => {
        this.#updateState({ MEDIA_MICROPHONE_ACCESS: res === true })
      })
    }

    #getCameraAccess () {
      Pear.media.access.camera().then((res) => {
        this.#updateState({ MEDIA_CAMERA_ACCESS: res === true })
      })
    }
    ...
    async #restartPlatformTimeoutCheck () {
      this.#updateState({ LIFECYCLE_RESTART_PLATFORM_TIMEOUT: true })
    }
