  ...
  class extends HTMLElement {
    constructor () {
      ...
      versions().then(({ platform }) => {
        ...
        if (this.state.restartPlatformPid) {
          this.#updateState({ LIFECYCLE_RESTART_PLATFORM: this.state.restartPlatformPid !== process.pid })
        }
        if (this.state.restartClientPid) {
          this.#updateState({ LIFECYCLE_RESTART_CLIENT: this.state.restartClientPid !== process.pid })
        }
    ...
    async #restartPlatform () {
      this.#updateState({ restartPlatformPid: process.pid })
      await Pear.restart({ platform: true })
    }

    async #restartClient () {
      this.#updateState({ restartClientPid: process.pid })
