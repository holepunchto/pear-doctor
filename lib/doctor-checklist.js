            ...
            ${section.items
              .map(
                ...
                <div class="list-item ${!this.state.started || item.active === false ? 'inactive' : ''}" style="margin-left: 20px;">
                  <input type="checkbox" id="item-${index}" ${this.state[item.id] === true ? 'checked' : ''} class="check-item ${this.state[item.id] === false && item.active !== false ? 'red-cross' : ''}" ${!this.state.started ? 'disabled' : ''}>
