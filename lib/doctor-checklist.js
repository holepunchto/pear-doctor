...
customElements.define(
  ...
  class extends HTMLElement {
    constructor () {
      ...
      versions().then(({ platform }) => {
        ...
        wakeups((wakeup) => {
          ...
          Runtime.Window.self.focus({ steal: true }).catch(console.error)
        })
      })
    }
    ...
    async #workerParentClose () {
      ...
      setTimeout(() => Runtime.Window.self.quit(), 2000)
    }
    ...
    async #userAgentCheck () {
      ...
      this.#updateState({ MISC_USER_AGENT: text === Pear.config.options.userAgent })
    }
