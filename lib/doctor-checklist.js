...
customElements.define(
  ...
  class extends HTMLElement {
      ...
      versions().then(async ({ platform }) => {
          ...
          if (this.entrypoint?.length > 1) {
            this.#updateState({
              COLD_START_ENTRYPOINT_FRAGMENT:
                this.#isValidEntrypoint(config.entrypoint) && this.#isValidFragment(config.fragment)
            })
          }
        }
          ...
          if (this.entrypoint?.length > 1) {
            this.#updateState({
              COLD_START_ENTRYPOINT_QUERY:
                this.#isValidEntrypoint(config.entrypoint) && this.#isValidQuery(config.query)
            })
          }
        }
        ...
        wakeups((wakeup) => {
          ...
          if (typeof wakeup.linkData === 'string' && isFromInside) {
            this.#updateState({
              WAKE_UP_LINK_FROM_INSIDE_APP: this.#isValidLinkData(wakeup.linkData)
            })
          }
      ...
      <div class="section ${!this.state.started ? 'inactive' : ''}">
        ...
        ${section.items
          .map((item, index) => {
            ...
            const isActive =
              !this.platformLength || Number(this.platformLength) > item.activeFromLength
    ...
    #attachEventListeners() {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach((button) => {
        if (this.eventListeners.has(button)) {
          button.removeEventListener('action-button-click', this.eventListeners.get(button))
        }
      ...
      if (this.eventListeners.has('contextmenu')) {
        this.shadowRoot.removeEventListener('contextmenu', this.eventListeners.get('contextmenu'))
      }
    ...
    #storageCheck() {
      ...
      this.#updateState({
        STORAGE:
          expectedStorageDir === currentStorageDir && (Pear.key ? dkey === storageFolder : true)
      })
    }
