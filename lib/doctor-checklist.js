...
customElements.define(
  ...
  class extends HTMLElement {
    constructor () {
      ...
      this.storage = new Localdrive('./userData')
      ...
      versions().then(async ({ platform }) => {
        ...
        for await (const file of this.storage.list()) {
          if (!file.key.includes(platformVersion)) {
            await this.storage.del(file.key)
          }
        }
        ...
        wakeups((wakeup) => {
          ...
          ui.app.focus({ steal: true }).catch(console.error)
        })

        this.state['WORKER_PARENT_CRASH'] === 'started' && this.#workerParentCrashCheck()
        this.state['WORKER_PARENT_CLOSE'] === 'started' && this.#workerParentCloseCheck()
      })
    }
    ...
    async #loadState () {
      const chunks = []
      try {
        const rs = this.storage.createReadStream('/' + this.storageKey + '.json')
        for await (const chunk of rs) {
          chunks.push(chunk)
        }
      } catch {
        console.log('No states stored')
      }
      const buffer = Buffer.concat(chunks)
      const state = buffer && buffer.length > 0 ? buffer.toString() : null
      return state ? JSON.parse(state) : { started: false }
    }

    #saveState () {
      this.storage.put('/' + this.storageKey + '.json', Buffer.from(JSON.stringify(this.state)))
    }
    ...
    #attachEventListeners () {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach(button => {
        ...
        const listener = (event) => {
          ...
          if (type === 'Teardown-Normal' && action === 'Setup') this.#teardownNormal()
          if (type === 'Teardown-Reject' && action === 'Setup') this.#teardownReject()
          if (type === 'Teardown-Force' && action === 'Setup') this.#teardownForce()
          ...
          if (type === 'Update-App-Desktop' && action === 'Check') this.#updateAppDesktopTest()
          ...
          if (type === 'Worker-Parent-Close' && action === 'Check') this.#workerParentClose()
          if (type === 'Worker-Parent-Crash' && action === 'Check') this.#workerParentCrash()

          if (type === 'Tray-Default' && action === 'Check') this.#trayDefaultRun()
          if (type === 'Tray-Custom-Icon' && action === 'Check') this.#trayCustomIconRun()
          if (type === 'Tray-Custom-Menu' && action === 'Check') this.#trayCustomMenuRun()
          if (type === 'Tray-Multiple-Runs' && action === 'Check') this.#trayMultipleRunsRun()
    ...
    #teardownReject () {
      teardown(() => new Promise((resolve, reject) => {
        ...
        this.#updateState({ LIFECYCLE_TEARDOWN_REJECT: true })
    ...
    async #updateAppDesktopTest () {
      ...
      await fs.promises.writeFile(path.join(tmpDir, 'app.js'), `
        ...
        await fs.promises.writeFile(path.join('${sanitizePath(tmpDir)}', '${Date.now()}.tmp'), '${Date.now()}')
        Pear.exit()
      `)
      ...
      const runChild = spawn(bin, ['run', tmpDir], { shell: !!isWindows })
      ...
      if (tmpFiles.length > 0) {
        ...
        throw new Error('❌ Update did not trigger — no temp files found in app.')
      }
    }
    ...
    async #workerParentClose () {
      this.#updateState({ WORKER_PARENT_CLOSE: 'started' })
    ...
    async #workerParentCrash () {
      this.#updateState({ WORKER_PARENT_CRASH: 'started' })
    ...
    async #trayDefaultRun () {
      ...
      let tray
      try {
        tray = await ui.app.tray()
        console.log(tray)
        this.#updateState({ TRAY_DEFAULT: true })
      } catch (err) {
        throw new Error(`Couldnt access default Tray: ${tray}\n Error: ${err}`)
      }
    }
    ...
    async #trayCustomIconRun () {
      ...
      let tray
      try {
        tray = await ui.app.tray({ icon: 'assets/Pear-white-16px.png' })
        console.log(tray)
        this.#updateState({ TRAY_CUSTOM_ICON: true })
      } catch (err) {
        throw new Error(`Couldnt access custom Tray: ${tray}\n Error: ${err}`)
      }
    }
    ...
    async #trayCustomMenuRun () {
      ...
      let tray
      try {
        tray = await ui.app.tray({ menu: { foo: 'Foo', bar: 'Bar' } }, (key) => {
        ...
        throw new Error(`Couldnt access custom menu Tray: ${tray}\n Error: ${err}`)
      }
    }
    ...
    async #trayMultipleRunsRun () {
      ...
      const trayOpts = [
        [undefined, undefined],
        [{ icon: 'assets/Pear-white-16px.png' }, undefined],
        [{ menu: { foo: 'Foo', bar: 'Bar' } }, undefined],
        [
          {
      ...
      let tray
      try {
        for (const [opt, handler] of trayOpts) {
          tray = await ui.app.tray(opt, handler)
          console.log('Tray created:', tray)
        }
        this.#updateState({ TRAY_MULTIPLE_RUNS: true })
      } catch (err) {
        throw new Error(`Couldn't access custom menu Tray: ${tray}\nError: ${err}`)
      }
    }
