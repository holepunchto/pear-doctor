...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    #attachEventListeners () {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach(button => {
        ...
        const listener = (event) => {
          ...
          if (type === 'Application' && action === 'Update') this.#applicationUpdate()
          if (type === 'Platform' && action === 'Update') this.#platformUpdate()
          if (type === 'Open' && action === 'Update') this.#applicationUpdate()
          ...
          if (type === 'Restart-Platform' && action === 'Check') this.#restartPlatform()
          if (type === 'Restart-Client' && action === 'Check') this.#restartClient()
        }
        ...
        <body>
          ...
          <div>
            ...
            <ul id="link-list">
              <li>
                <strong>Link:</strong>
                <a id="link1" href="${config.applink}">${config.applink}</a>
              </li>
              <li>
                <strong>Fragment:</strong>
                <a id="link2" href="${config.applink}/#fragment">${config.applink}/#fragment</a>
              </li>
              <li>
                <strong>Entrypoint:</strong>
                <a id="link3" href="${config.applink}/nested/entrypoint.html">${config.applink}/nested/entrypoint.html</a>
              </li>
              <li>
                <strong>Entrypoint fragment:</strong>
                <a id="link4" href="${config.applink}/nested/entrypoint.html#fragment">${config.applink}/nested/entrypoint.html#fragment</a>
              </li>
              <li>
                <strong>Legacy invite code:</strong>
                <a id="link5" href="${config.applink}/xeb7mugj8sbaytkf5qqu9z1snegtibqneysssdqu35em4zw3ou9wcmz8ha4er6e759tams9eeebo6j6ueifyb4oaeohnijbyxfzessxjneaqs8ux">${config.applink}/xeb7mugj8sbaytkf5qqu9z1snegtibqneysssdqu35em4zw3ou9wcmz8ha4er6e759tams9eeebo6j6ueifyb4oaeohnijbyxfzessxjneaqs8ux</a>
              </li>
              <li>
    ...
    async getSidecarPid () {
      const { error, result } = await this.sidecars({ name: 'pear-runtime', flag: '--sidecar' })
      if (error) {
        console.error('Failed to get sidecars', error)
        return null;
      }
      if (result.sidecars.length !== 1) { 
        console.error('Expected 1 sidecar, but got', result.sidecars.length)
        return null
      }
      const sidecarPid = result.sidecars[0]
      if (!sidecarPid) {
        console.error('Expected sidecar pid, but got', sidecarPid)
        return null
      }
      return sidecarPid
    }
    ...
    #checkTrustDialog () {
      const permitsPath = path.join(Pear.config.pearDir, 'permits.json')
      const permits = JSON.parse(fs.readFileSync(permitsPath, 'utf8'))
      const z32 = encode(Buffer.from(this.randomKey, 'hex'))
      const trusted = permits.trusted.includes(z32)
      this.#updateState({ LIFECYCLE_TRUST_DIALOG: trusted })
    }
    ...
    async #restartPlatform () {
      const sidecarPid = await this.getSidecarPid()
      if (!sidecarPid) {
        this.#updateState({ LIFECYCLE_RESTART_PLATFORM: false })
        return
      }
      const sidecarPidKey = 'LIFECYCLE_RESTART_PLATFORM_sidecarPid';
      const lastSidecarPid = this.state[sidecarPidKey]
      this.#updateState({ [sidecarPidKey]: sidecarPid })
      ...
      // check
      if (!this.state.LIFECYCLE_RESTART_PLATFORM) {
        this.#updateState({ LIFECYCLE_RESTART_PLATFORM: lastSidecarPid !== sidecarPid })
      }
    }
    ...
    async #restartClient () {
      const sidecarPid = await this.getSidecarPid()
      if (!sidecarPid) {
        this.#updateState({ LIFECYCLE_RESTART_CLIENT: false })
        return
      }
      const sidecarPidKey = 'LIFECYCLE_RESTART_CLIENT_sidecarPid';
      const lastSidecarPid = this.state[sidecarPidKey]
      this.#updateState({ [sidecarPidKey]: sidecarPid })
      ...
      // check
      if (!this.state.LIFECYCLE_RESTART_CLIENT) {
        this.#updateState({ LIFECYCLE_RESTART_CLIENT: lastSidecarPid !== sidecarPid })
      }
    }
        ...
        <div id="panel">
          ...
          <div class="list-container">
            <div class="header-container">
              ...
              <action-button icon="✚" action="" type="Nurse" applink="${config.applink}" ></action-button>
