...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    async #updatePlatform () {
      ...
      const dumpDir = isPearDev ? config.pearDir : path.join(tmp, 'pear-doctor-update-platform-dump')
      ...
      const { kill } = await bootstrapPlatforms({
    ...
    async #updatePlatformDesktopTest () {
      ...
      this.#updateState({ [statusId]: undefined })
...
const platformDir1SeedApp = async ({ onProgress, platformDir1, appDir }) => {
  ...
  return { appLink: stageOut.link, kill: () => seedChild.kill() }
}
