...
customElements.define(
  ...
  class extends HTMLElement {
    constructor () {
      ...
      //global.pipe = Pear.worker.run(`${Pear.config.applink}/worker`)
      global.pipe = Pear.worker.run(`${Pear.config.links.worker}`)
      this.pipe = global.pipe

      console.log('config.applink:', config.applink)
      this.rpc = new RPC((data) => {
      ...
      this.workerApi = new Map()
      this.workerApi.set('sidecars', this.rpc.register(0, { request: any, response: any }))
    ...
    async #sidecars () {
      ...
      let { result, error } = await this.workerApi.get('sidecars').request({ name: 'pear-runtime', flag: '--sidecar' })
      if (error) return console.error(error)

      const { sidecars } = result
      this.#updateState({ LIFECYCLE_ZOMBIE_SIDECARS: sidecars.length <= 1 })

      const text = `Total sidecar processes: ${sidecars.length} (pid ${sidecars.join(', ')})`;
