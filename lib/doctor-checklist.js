...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    async #updatePlatform () {
      const { dumpDir, platformDir } = await preparePlatform()
      const { appLink } = await prepareApp(platformDir)
      const { pipe } = await runApp(appLink, platformDir)
      ...
      this.#updateState({ LIFECYCLE_UPDATE_PLATFORM: true })
      ...
      async function preparePlatform () {
        ...
        console.log('dumping...', version.platform.key, dumpDir)
        ...
        const platformDir = path.join(tmp, 'pear-doctor-update-platform-dir')
        await fs.promises.rm(platformDir, { recursive: true, force: true })
        await fs.promises.mkdir(platformDir, { recursive: true })
        console.log('bootstrapping...', dumpKey, platformDir)
        await pearUpdaterBootstrap(dumpKey, platformDir, { bootstrap: config.dht.bootstrap })

        return { dumpDir, platformDir }
      }

      async function prepareApp (platformDir) {
        ...
        const platformRuntime = path.join(platformDir, 'current', BY_ARCH)
      ...
      async function runApp (appLink, platformDir) {
        const workerRuntime = Pear.worker.constructor.RUNTIME
        const platformRuntime = path.join(platformDir, 'current', BY_ARCH)
        Pear.worker.constructor.RUNTIME = platformRuntime
        const pipe = Pear.worker.run(appLink)
        Pear.worker.constructor.RUNTIME = workerRuntime
        await pipeUntil({ pipe, tag: 'versions' })
