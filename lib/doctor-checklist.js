...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    async #updateAppDesktopTest () {
      ...
      `)
      const electronVersion = await getDepVersion('pear-electron')
      const bridgeVersion = await getDepVersion('pear-bridge')
      await fs.promises.writeFile(path.join(tmpDir, 'package.json'), JSON.stringify({
        name,
        type: 'module',
        main: 'index.js',
        ...
        dependencies: {
          'pear-bridge': bridgeVersion,
          'pear-electron': electronVersion
        }
      }))
    ...
    async #updatePlatformDesktopTest () {
      ...
      const createApp = async () => {
        ...
        const electronVersion = await getDepVersion('pear-electron')
        const bridgeVersion = await getDepVersion('pear-bridge')
        const assets = await getAssetsConfig()
        await fs.promises.writeFile(path.join(appDir, 'package.json'), JSON.stringify({
          name,
          type: 'module',
          main: 'index.js',
...
async function getDepVersion (dep){
  const __dirname = fileURLToPath(Pear.app.applink)
...
async function getAssetsConfig (){
  ...
  const pkgPath = path.join(__dirname, 'node_modules', 'pear-electron', 'package.json')
  const pkg = JSON.parse(await fs.promises.readFile(pkgPath, 'utf8'))

  return pkg.pear.assets
}
