...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    async #updatePlatform () {
      const { appLink } = await prepareApp()
      const { platformDir, dumpDir } = await preparePlatform()
      ...
      async function prepareApp () {
        ...
        const stageChild = spawn(bin, ['stage', '--json', 'dev', appDir])
        teardown(async () => stageChild.kill())
        const stageOut = await spawnUntil({ sc: stageChild, tag: 'addendum' })

        const seedChild = spawn(bin, ['seed', '--json', 'dev', appDir])
