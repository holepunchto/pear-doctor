...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    async #workerParentEnd () {
      const appDir = await createWorkerChildBlank('end')

      const pipe = Pear.worker.run(appDir)
      pipe.resume()
      const untilClosePromise = pipeUntilClose({ pipe })
      await new Promise((resolve) => setTimeout(resolve, 1000))
      pipe.end()
      await untilClosePromise
      try {
    ...
    async #workerParentDestroy () {
      const appDir = await createWorkerChildBlank('destroy')
...
async function createWorkerChildBlank (suffix) {
  const name = `pear-doctor-worker-child-${suffix}`
  const appDir = path.join(tmp, name)
  await fs.promises.rm(appDir, { recursive: true, force: true })
  await fs.promises.mkdir(appDir, { recursive: true })
  await fs.promises.writeFile(path.join(appDir, 'index.js'), `
