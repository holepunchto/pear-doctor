...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    async #workerParentClose () {
      console.debug('Writing worker code')
      ...
      await fs.promises.writeFile(path.join(dir, 'index.js'), `
        ...
        Pear.teardown(() => console.log('Worker teardown'))
        ...
        let interval
        pipe.on('data', (data) => {
          if (data.toString() === 'start') interval = setInterval(() => fs.appendFileSync(outputPath, (count++).toString() + '\\n', 'utf8'), 1000)
        })

        pipe.on('end', () => {
          console.log('Pipe ending')
          interval && clearInterval(interval)
          fs.appendFileSync(outputPath, 'ended\\n', 'utf8')
        })
      ...
      console.debug('Installing npm dependencies')
      const npmi = spawn('npm', ['install'], { cwd: dir, stdio: ['pipe', 'pipe'] })
      npmi.stdout.on('data', data => console.log('stdout:', data.toString()))
      npmi.stderr.on('data', data => console.error('stderr:', data.toString()))
      await new Promise((resolve, reject) => npmi.on('exit', (code) => code === 0 ? resolve(code) : reject(code)))

      console.debug('Running worker')

      const workerPipe = Pear.worker.run(dir)
      await new Promise(resolve => workerPipe.once('data', resolve))
