    ...
    #renderChecklist () {
      ...
      <div class="section ${!this.state.started ? 'inactive' : ''}">
        ...
        ${section.items
          .map(
            (item, index) => `
            <div class="list-item ${!this.state.started || item.active === false ? 'inactive' : ''}" style="margin-left: 20px;">
              <input type="checkbox" id="item-${index}" ${this.state[item.id] === true && item.active !== false ? 'checked' : ''} class="check-item ${this.state[item.id] === false && item.active !== false ? 'red-cross' : ''}" ${!this.state.started ? 'disabled' : ''}>
              ...
              ${item.link ? `<a href="${this.#parseLink(item.link)}" target="_blank" class="link">Link</a>` : ''}
