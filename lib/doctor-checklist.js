...
const bin = isLocal
  ...
  : path.join(config.pearDir, 'bin', isWindows ? 'pear.exe' : 'pear')
        ...
        ${section.items
          .map(
            ...
            <div class="list-item " style="margin-left: 1.25rem;">
              ...
              ${item.link ? `<action-button action="Link" type="Click" href="${item.link}" class="${!this.state.started || !isActive || this.state[`loading-${item.id}`] ? 'inactive' : ''}" applink="${applink}"></action-button>` : ''}
              ${item.button ? `<${item.button} id="${item.id}" action="${item.action}" type="${item.type}" class="${!this.state.started || !isActive || this.state[`loading-${item.id}`] ? 'inactive' : ''}"></${item.button}>` : ''}
              ${item.buttonSecond ? `<${item.buttonSecond} id="${item.id}" action="${item.actionSecond}" type="${item.typeSecond}" class="secondary-button${!this.state.started || !isActive || this.state[`loading-${item.id}`] ? ' inactive' : ''}"></${item.buttonSecond}>` : ''}
            </div>
    ...
    #attachEventListeners () {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach(button => {
        ...
        const listener = async (event) => {
          ...
          this.#updateState({ [`loading-${id}`]: true })
