...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    async #updateApp () {
      console.log('🚀 ~ #updateApp ~ start', Pear.config)

      const bin = path.join(config.pearDir, 'bin', `pear${isWindows ? '.exe' : ''}`)
      
      const tmpDir = path.join(tmp, 'pear-doctor-update-app')
      const name = 'pear-doctor-update-app' + Date.now()
      console.log('creating dir', tmpDir)
      await fs.promises.rm(tmpDir, { recursive: true, force: true })
      await fs.promises.mkdir(tmpDir, { recursive: true })
      await fs.promises.writeFile(path.join(tmpDir, 'index.js'), `Pear.updates((data) => { console.log(JSON.stringify({ tag: 'update', data })) })`)
      await fs.promises.writeFile(path.join(tmpDir, 'package.json'), JSON.stringify({ name, main: 'index.js', pear: { name } }))

      console.log('staging', bin, tmpDir)
      const stageChild = spawn(bin, ['stage', 'dev', '--json', tmpDir])
      const stageOut = await this.spawnUntil(stageChild, 'addendum')
      console.log('stageOut', stageOut)

      const link = stageOut.link

      console.log('seeding', bin, tmpDir)
      const seedChild = spawn(bin, ['seed', 'dev', '--json', tmpDir])
      const announced = await this.spawnUntil(seedChild, 'announced')
      console.log('announced', announced)

      console.log('running', bin, link)
      const runChild = spawn(bin, ['run', link])
      const updatePromise = this.spawnUntil(runChild, 'update')

      console.log('writing test file')
      await fs.promises.writeFile(path.join(tmpDir, `${Date.now()}.tmp`), `${Date.now()}`)

      console.log('staging again', bin, tmpDir)
      const stageChild2 = spawn(bin, ['stage', 'dev', '--json', tmpDir])
      const stageOut2 = await this.spawnUntil(stageChild2, 'addendum')
      console.log('stageOut2', stageOut2)

      const updated = await updatePromise
      console.log('🚀 ~ #updateApp ~ done', updated)

      if (stageOut2.version === updatePromise.version.length) {
        this.#updateState({ LIFECYCLE_UPDATE_APP: true })
      }
    }

    async spawnUntil (sc, tag) {
