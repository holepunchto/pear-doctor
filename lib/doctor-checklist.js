  ...
  class extends HTMLElement {
    ...
    #attachEventListeners() {
      ...
      this.shadowRoot.querySelectorAll('action-button').forEach((button) => {
        ...
        const listener = (event) => {
          ...
          if (type === 'Trust-Dialog' && action === 'Check')
            this.#checkTrustDialog()
          if (type === 'Password-Dialog' && action === 'Check')
            this.#checkPasswordDialog()
          ...
          if (type === 'Worker-Subprocess' && action === 'Check') this.#workerSubprocess()
    ...
    async #updateApp() {
      ...
      await fs.promises.writeFile(path.join(tmpDir, 'package.json'), JSON.stringify({
        ...
        main: 'index.js',
        pear: {},
        dependencies: { 'pear-pipe': 'latest', 'pear-updates': 'latest' }
      }))

      const npmi = spawn('npm', ['install'], { cwd: tmpDir, stdio: 'ignore', shell: !!isWindows })
      ...
      await fs.promises.writeFile(
        path.join(tmpDir, `${Date.now()}.tmp`),
        `${Date.now()}`
      )

      const updateRes = await updatePromise
      const isUpdated =
        updateRes.type === 'pear/updates' && updateRes.app === true
    ...
    async #updateAppDesktopTest() {
      ...
      await fs.promises.writeFile(path.join(tmpDir, 'package.json'), JSON.stringify({
        ...
        dependencies: { 'pear-bridge': 'latest', 'pear-electron': 'latest', 'pear-updates': 'latest' }
      }))
    ...
    async #updatePlatform() {
      ...
      const dumpDir = isPearDev
        ? path.join(config.pearDir, '..')
        : path.join(tmp, 'pear-doctor-update-platform-dump')
      ...
      const platformDir2RunApp = async (appLink) => {
        ...
        await fs.promises.writeFile(path.join(tmpDir, 'package.json'), JSON.stringify({
          ...
          main: 'index.js',
          pear: {},
          dependencies: { 'pear-pipe': 'latest', 'pear-run': 'latest' }
        }))
        ...
        // we run a terminal app `tmpDir` to run the `appLink` instead
        const pipe = run(tmpDir)
        teardown(async () => pipe.end())
        await pipeUntil({ pipe, tag: 'versions' })
      ...
      const stageNewFile = async () => {
        await fs.promises.writeFile(
          path.join(dumpDir, `${Date.now()}.tmp`),
          `${Date.now()}`
        )
      ...
      const { kill: killSeedPlatform } = await bootstrapPlatforms({
        ...
        onDone: (result) =>
          this.#updateState({ LIFECYCLE_UPDATE_PLATFORM: result }),
        dumpDir,
        platformDir1,
        platformDir2
      })
      const { tmpDir } = await createApp()
      ...
      const updateRes = await updatePromise
      const isUpdated =
        updateRes.type === 'pear/updates' && updateRes.app === false

      this.#updateState({
        LIFECYCLE_UPDATE_PLATFORM: isUpdated,
        [statusId]: undefined
      })
    ...
    async #updatePlatformDesktopTest() {
      ...
      const { kill: killSeedPlatform } = await bootstrapPlatforms({
        onProgress: (progress) => this.#updateState({ [statusId]: progress }),
        onDone: (result) =>
          this.#updateState({ LIFECYCLE_UPDATE_PLATFORM_DESKTOP: result }),
        dumpDir,
        platformDir1,
        platformDir2
      })
      ...
      this.#updateState({
        [statusId]: undefined,
        'pid-LIFECYCLE_UPDATE_PLATFORM_DESKTOP': pid
      })
    ...
    async #workerSubprocess () {
      ...
      const link = Pear.key ? `${Pear.config.applink}/subprocess/index.js` : path.join(Pear.config.dir, 'subprocess', 'index.js')
      const pipe = run(link)
      pipe.once('data', (data) => {
    ...
    async #workerParentClose () {
      ...
      await fs.promises.writeFile(path.join(tmpDir, 'package.json'), JSON.stringify({
        ...
        main: 'index.js',
        pear: { type: 'terminal' },
        dependencies: { 'bare-fs': 'latest', 'bare-path': 'latest', 'pear-pipe': 'latest' }
      }))
...
async function bootstrapPlatforms({
  onProgress,
  onDone,
  dumpDir,
  platformDir1,
  ...
  if (!isPearDev) {
    ...
    const dumpChild = spawn(
      bin,
      ['dump', '--json', `pear://${version.platform.key}`, dumpDir],
      { shell: !!isWindows }
    )
  ...
  const stageChild = spawn(bin, ['stage', '--json', 'dev'], { cwd: dumpDir, shell: !!isWindows })
  teardown(async () => stageChild.kill())
  const stageOut = await spawnUntil({ sc: stageChild, tag: 'addendum' })
  const dumpKey = stageOut.key

  onProgress('seeding-platform...')
  const seedChild = spawn(bin, ['seed', '--json', 'dev'], { cwd: dumpDir, shell: !!isWindows })
  ...
  await pearUpdaterBootstrap(dumpKey, platformDir1, {
  ...
  await pearUpdaterBootstrap(dumpKey, platformDir2, {
    bootstrap: config.dht.bootstrap
  })
...
async function platformDir1SeedApp({ onProgress, platformDir1, tmpDir }) {
  const platformRuntime = path.join(platformDir1, 'current', BY_ARCH)

  onProgress('staging-app...')
  const stageChild = spawn(platformRuntime, ['stage', '--json', 'dev'], { cwd: tmpDir, shell: !!isWindows })
  teardown(async () => stageChild.kill())
  const stageOut = await spawnUntil({ sc: stageChild, tag: 'addendum' })

  onProgress('seeding-app...')
  const seedChild = spawn(platformRuntime, ['seed', '--json', 'dev'], { cwd: tmpDir, shell: !!isWindows })
  teardown(async () => seedChild.kill())
  await spawnUntil({ sc: seedChild, tag: 'announced' })
